...
export class ResultsService {
  static async getExamResults(examId: string): Promise<UnifiedResultRow[]> {
    const payload = await ResultsService.getUnifiedResults(examId);
  ...
  static async getUnifiedResults(
    ...
    // 1. Load Classes and Exams from optimized services (Local-First)
    const { ClassService } = await import("./classService");
    const { ExamService } = await import("./examService");
    ...
    const scanDocs = Array.from(cachedGrades).map((g: any) => ({
      ...
      data: {
        ...
        totalQuestions: g.totalPoints, // Alias for processing
    ...
    // 3. Combine with Offline Grades from Staging
    ...
    offlineGrades.forEach((og: any) => {
      ...
      rows.push({
        ...
        classLabel: examMeta?.classLabel || "Local Storage",
        score,
        totalQuestions,
        percentage,
        dateValue,
        correctLabel: `${score}/${totalQuestions} correct`,
        sortValue: new Date(dateValue).getTime() || Date.now(),
      });
